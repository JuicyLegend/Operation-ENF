function autoArrangeFigures(NH, NW, monitors, monitor_id)
% INPUT  :
...
if monitors >= 2
    ...
    for j=1:monitors
        ...
        screen_sz = get(0,'MonitorPositions');
        screen_sz = screen_sz(j, :);
        scn_w = screen_sz(3) - task_bar_offset(1);
        scn_h = screen_sz(4) - task_bar_offset(2);
        scn_w_begin = screen_sz(1) + task_bar_offset(1);
        scn_h_begin = screen_sz(2) + task_bar_offset(2);
        if autoArrange==0
            if n_fig > N_FIG && N_FIG == 0
                autoArrange = 1;
                warning('too many figures than you told. change to autoArrange 1');
            else
                nh = NH;
                nw = NW;
            end
